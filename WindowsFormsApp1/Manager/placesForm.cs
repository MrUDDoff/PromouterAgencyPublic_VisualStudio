...
namespace WindowsFormsApp1.Manager
{
    public partial class placesForm : Form
    {
        ...
        private void placesForm_Activated(object sender, EventArgs e)
        {
            DB db = new DB();
        ...
        {
            newstockForm newstockform = (newstockForm)this.Owner;

            string nameStock = newstockform.nameField.Text;

            DB db = new DB();

            SqlDataAdapter adapter = new SqlDataAdapter();

            SqlCommand findstockcommand = new SqlCommand("SELECT * FROM Акции WHERE наименование = @NS", db.getConnection());
            findstockcommand.Parameters.Add("@NS", SqlDbType.NVarChar).Value = nameStock;
            adapter.SelectCommand = findstockcommand;

            DataTable findstocktable = new DataTable();
            adapter.Fill(findstocktable);

            int idStock = findstocktable.Rows[0].Field<int>("idАкции");
            ...
            foreach (object item in listBoxplacesforstock.Items)
            {
                ...
                newcommand.Parameters.Add("@IS", SqlDbType.Int).Value = idStock;
